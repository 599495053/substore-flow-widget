...
const flows = {
  ...
  '机场B': { status: 'success', data: { total: 53687091200, usage: { upload: 0, download: 48318382080 }, expires: Math.floor(Date.now()/1000)+86400*5, remainingDays: 1 } },
...
const ctx = {
  ...
  http: { get: async (url) => {
    let body;
    if (url.endsWith('/api/subs')) body = { status: 'success', data: Object.keys(flows).map(name => ({ name, url: 'https://example.com/sub#resetDay=1' })) };
    else {
      const name = decodeURIComponent(url.split('/api/sub/flow/')[1]);
      body = flows[name] || { status: 'failed', error: { message: 'not found' } };
    }
    return { status: 200, text: async () => JSON.stringify(body) };
  } }
};

const out = await widget(ctx);
